...
//    $("#setchangecdnBtn").attr("disabled", true)
    $("#setreturncdnBtn").attr("disabled", true)
    ...
    $("#checkcdnBtn").click(function() {
        checkcdnconfig()
//        $("#setchangecdnBtn").attr("disabled", true)
        $("#setreturncdnBtn").attr("disabled", true)
    })

    $("#setchangecdnBtn").click(function() {
        setchangecdn()
    })

    $("#setreturncdnBtn").click(function() {
        setreturncdn()
    })
...
function changeActiveMenu(path){
        ...
        } else if(path.match('/cdn')){
            $('li').removeClass('active')
            $('#cdnMenu').addClass('active')
        }
}
...
function createSwapResultTable(res){
    ...
    ret += '<table class="table table-bordered"><thead><tr><th class="titleText">FQDN</th><th class="titleText">dig @11ST DNS Servers</th><th class="titleText">Records In Zonefile</th>/tr></thead><tbody>' 
    for (fqdn in res){
    ...
    ret += "</tbody></table>"
    for (fqdn in res){
//        dig = res[fqdn]['dig'].split('\n')[0].replace('ns1 : ', '') // original conf.
        var re = /[A-Za-z]*\-[A-Za-z]*[0-9]*\s\:\s/;
        dig = res[fqdn]['dig'].split('\n')[0].replace(re, '')
        a = 24 - fqdn.length
        line = fqdn
        while(a>0){
...
function createResultTable(res){
    ret = "<label>Result</label>"
    ret += '<table class="table table-bordered"><thead><tr><th class="titleText">FQDN</th><th class="titleText">dig @11ST DNS Servers</th><th class="titleText">Records In Zonefile</th><th><button type="button" class="btn btn-danger" id="deleteBtn">선택 삭제</button></th></tr></thead><tbody>' 
    for (fqdn in res){
        rows = res[fqdn]['record'].length
        ret += '<tr>'
        if(rows > 1){
    ...
    ret += "{code}\n"
    for (fqdn in res){
//        dig = res[fqdn]['dig'].split('\n')[0].replace('ns1 : ', '') // 이 줄을 주석 처리하고 아래 두줄 허용. // original conf.
        var re = /[A-Za-z]*\-[A-Za-z]*[0-9]*\s\:\s/; // jw. dig 기능 개선.
        dig = res[fqdn]['dig'].split('\n')[0].replace(re, '') // jw. dig 기능 개선.
        a = 24 - fqdn.length
        line = fqdn
        while(a>0){
            line += ' '
...
function setchangecdn(res){
    var action = 'setchangeRecords'
    var data = {
        action: action
    }
    data = JSON.stringify(data)
...
function setreturncdn(res){
    return ret
}


function checkcdnconfig(res){
    var action = 'checkcdnconfig'
    var data = {
        action: action
    }
    data = JSON.stringify(data)
...
function createResultTableForCdn(res){
    ret = "<label>Current Result</label>"
    ret += '<table class="table table-bordered"><thead><tr><th class="titleText"></th><th class="titleText">FQDN</th><th class="titleText">Records In Zonefile</th></tr></thead><tbody>'
    for (fqdn in res){
        rows = res[fqdn]['record'].length
        ret += '<tr>'
        if(rows > 1){
            ret += '<td>' + res[fqdn]['dns'] + '</td>' + '<td rowspan=' + rows + '>' + fqdn + '</td>'
            for(i in res[fqdn]['record']){
                record = res[fqdn]['record'][i]
                if(i == 0){
                    ret += '<td><div class="pre">' + record + '</div></td></tr>'
            ...
            ret += '<td>' + res[fqdn]['dns'] + '</td>' + '<td>' + fqdn + '</td>'
            record = res[fqdn]['record'][0]
            if(record.indexOf('ERROR') == -1){
                ret += '<td><div class="pre">' + record + '</div></td></tr>'
            } else {
